...
export function patrol(speed = 60, dir = (-1) ** get("platform").length) {
  return {
    id: "patrol",
  ...
  return {
    ...
    add() {
      onMouseRelease(() => {
        ...
        if (!this.level) this.level = this.getLevel();
      ...
      add() {
        ...
        this.onGround(() => {
          ...
          if (!this.curDragging()) {
            ...
            // for (let i = -1; i <= 1; i += 2) {
            //   this.level.spawn("=", this.tilePos.add(vec2(i, -1)));
            // }
          }
        })
      }
